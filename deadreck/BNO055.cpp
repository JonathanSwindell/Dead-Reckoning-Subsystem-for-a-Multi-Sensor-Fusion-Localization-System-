...
void BNO055::readAbsAcc(){
    ...
    /*
    tempQuat[0]=0 -accel.x*quat.q1 -accel.y*quat.q2 -accel.z*quat.q3;
    tempQuat[1]=0 +accel.x*quat.q0 -accel.y*quat.q3 +accel.z*quat.q2;
    tempQuat[2]=0 +accel.x*quat.q3 +accel.y*quat.q0 -accel.z*quat.q1;
    tempQuat[3]=0 -accel.x*quat.q2 +accel.y*quat.q1 +accel.z*quat.q0;
    */
...
void BNO055::resetDeadReckoning() {
    aXOld = 0.0;
    aYOld = 0.0; 
    aZOld = 0.0;
    vXOld = 0.0; 
...
void BNO055::deadReckoning(int mode){
    ...
    float interval=((float)timeNowDeadReckoning-(float)timeOldDeadReckoning)/1000.0;
...
void BNO055::readCalibrationStatus(){
        ...
        calibration.sysSta = (calibrationStatus) ((rawData[0] & BNO055_SYS_CALIB_STAT__MSK) >> BNO055_SYS_CALIB_STAT__POS);
        calibration.gyroSta = (calibrationStatus) ((rawData[0] & BNO055_GYR_CALIB_STAT__MSK) >> BNO055_GYR_CALIB_STAT__POS);
        calibration.accelSta = (calibrationStatus) ((rawData[0] & BNO055_ACC_CALIB_STAT__MSK) >> BNO055_ACC_CALIB_STAT__POS);
        calibration.magSta = (calibrationStatus) ((rawData[0] & BNO055_MAG_CALIB_STAT__MSK) >> BNO055_MAG_CALIB_STAT__POS);
}
